/* DESCRIPTION: E93 OS Interrupt Service Routine Source File
...
void isr_handler(registers_struct_type registers) {
    ...
    printk(interrupt_no_str);
    printk("\n");

    printk(isr_exception_messages[registers.interrupt_no]);
    printk("\n");
}
...
void irq_handler(registers_struct_type registers) {
    ...
    if (interrupt_handlers[registers.interrupt_no] != 0) {
        void * handler = interrupt_handlers[registers.interrupt_no];
        handler(registers);
    }
}
